...
export const generateMoves = (svg, xScale, moves) => {
    console.log(moves);
    for(let y = 0; y<8; y++) {
        for(let x = 0; x < 8; x++) {
            svg
                .selectAll(`.move_${String.fromCharCode(97 + x)}${y+1}`)
                .remove()

            moves.find(move => move.to === `${String.fromCharCode(97 + x)}${8-y}`) && svg
                .selectAll(`.move_${String.fromCharCode(97 + x)}${y+1}`)
                .data([null])
                .join('circle')
                .attr('class', `move_${String.fromCharCode(97 + x)}${y+1}`)
...
export const generatePiecesWhite = (svg, xScale, chess, pieces) => {
    ...
    for(let y = 0; y<8; y++) {
        for(let x = 0; x < 8; x++) {
            svg
                ...
                .on('click', () => {
                    generateMoves(svg, xScale, chess.moves({ verbose: true, square: `${String.fromCharCode(97 + x)}${8-y}` }))
                    // console.log(`${x}${y}`, chess.moves({ verbose: true, square: `${String.fromCharCode(97 + x)}${8-y}` }));
                })
        }
    }
}
